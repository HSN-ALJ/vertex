...
function WPrompt()
{
  ...
  var text_limit = 200;
  this.div_area.addEventListener("keypress", function(e) {
    if(this.value.length > text_limit) {
        e.preventDefault();
    }
  });

  this.div_area.addEventListener("keydown", function(e) {
    if(this.value.length > text_limit) {
        this.value = this.value.substring(0, text_limit);
    }
  });
  var _this = this;
